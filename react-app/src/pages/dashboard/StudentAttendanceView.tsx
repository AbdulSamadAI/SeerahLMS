    ...
    return (
        <div className="max-w-5xl mx-auto px-6 py-8">
            <header className="mb-10">
                ...
                <p className="text-slate-500 font-medium">Track your presence and commitment to the Prophetic path.</p>
            </header>

            {/* Stats Overview */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                <div className="card bg-white border-slate-100 shadow-xl p-6 rounded-[32px]">
                ...
                <div className="card bg-emerald-50 border-emerald-100/50 p-6 rounded-[32px]">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-white/50 flex items-center justify-center text-emerald-600">
                            <CheckCircle2 className="w-4 h-4" />
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600/60">Present</span>
                    </div>
                    <div className="text-3xl font-black text-emerald-700">{stats.present}</div>
                </div>

                <div className="card bg-rose-50 border-rose-100/50 p-6 rounded-[32px]">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-white/50 flex items-center justify-center text-rose-600">
                            <XCircle className="w-4 h-4" />
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-rose-600/60">Absent</span>
                    </div>
                    <div className="text-3xl font-black text-rose-700">{stats.absent}</div>
                </div>

                <div className="card bg-amber-50 border-amber-100/50 p-6 rounded-[32px]">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-white/50 flex items-center justify-center text-amber-600">
                            <Pause className="w-4 h-4" />
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-amber-600/60">Leave</span>
                    </div>
                        ...
                        {records?.map((record, index) => (
                            ...
                            >
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-6">
                                        <div className="w-16 h-16 rounded-2xl bg-primary-50 text-primary-600 flex flex-col items-center justify-center font-black shadow-inner">
                                            ...
                                            <span className="text-2xl">{record.class_number}</span>
                                        </div>
                                        <div>
                                            <h3 className="text-2xl font-black text-slate-900 mb-1">{record.session_topic || 'Regular Session'}</h3>
                                    ...
                                    <div className="hidden md:block">
                                        <div className={`px-6 py-2 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2 ${record.status === 'Present' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200' :
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div className={`p-4 rounded-2xl transition-all duration-500 ${record.status === 'Present'
                                        ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200 scale-[1.02]'
                                        : 'bg-slate-50 text-slate-400 opacity-40'
                                        }`}>
                                        <div className={`text-[10px] font-black uppercase tracking-widest mb-1 ${record.status === 'Present' ? 'text-white' : 'text-slate-400'}`}>Present</div>
                                        <div className="flex items-center justify-between">
                                            <div className="text-2xl font-black">{record.status === 'Present' ? 'YES' : '-'}</div>
                                            <CheckCircle2 className={`w-6 h-6 ${record.status === 'Present' ? 'text-white' : 'text-slate-200'}`} />
                                        </div>
                                    </div>

                                    <div className={`p-4 rounded-2xl transition-all duration-500 ${record.status === 'Absent'
                                        ? 'bg-rose-500 text-white shadow-lg shadow-rose-200 scale-[1.02]'
                                        : 'bg-slate-50 text-slate-400 opacity-40'
                                        }`}>
                                        <div className={`text-[10px] font-black uppercase tracking-widest mb-1 ${record.status === 'Absent' ? 'text-white' : 'text-slate-400'}`}>Absent</div>
                                        <div className="flex items-center justify-between">
                                            <div className="text-2xl font-black">{record.status === 'Absent' ? 'YES' : '-'}</div>
                                            <XCircle className={`w-6 h-6 ${record.status === 'Absent' ? 'text-white' : 'text-slate-200'}`} />
                                        </div>
                                    </div>

                                    <div className={`p-4 rounded-2xl transition-all duration-500 ${record.status === 'Leave'
                                        ? 'bg-amber-500 text-white shadow-lg shadow-amber-200 scale-[1.02]'
                                        : 'bg-slate-50 text-slate-400 opacity-40'
                                        }`}>
                                        <div className={`text-[10px] font-black uppercase tracking-widest mb-1 ${record.status === 'Leave' ? 'text-white' : 'text-slate-400'}`}>Leave</div>
                                        <div className="flex items-center justify-between">
                                            <div className="text-2xl font-black">{record.status === 'Leave' ? 'YES' : '-'}</div>
                                            <Pause className={`w-6 h-6 ${record.status === 'Leave' ? 'text-white' : 'text-slate-200'}`} />
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
